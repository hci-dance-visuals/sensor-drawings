...
#                    continue
    X_train = np.array(X_train)
    X_train = X_train.reshape([-1,res,res,1])/255.
    Y_train = np.array(Y_train)
    X_test = np.array(X_test)
    Y_test = np.array(Y_test)
#    X_test = X_train[int(round(X_train.shape[0]*0.9)):X_train.shape[0],:,:]
    X_test = X_test.reshape([-1,res,res,1])/255.
    return X_train, X_test, Y_train

def loadDatasetAugmented(res):
    exclude = set(["ipynb_checkpoints"])
    X_train = []
    Y_train = []
    X_augmented = []
    ...
    for path, subdirs, files, in os.walk(dataset_directory_2):
        ...
        for file in files:
            ...
            if "scaled" in file_path:
                id = str(int(ceil(len(X_train)/20)))
                label = get_name(file_path, 3) + '-' + "%02d" % int(seq_number%20)
                img = cv2.imread(file_path)
                img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
                X_train.append(img)
                Y_train.append(label)
                ...
                for i in range(num_augs):
                    aug_path = path+"/../augmented/"+append_id(file, str(i))
                    label = get_name(file_path, 3) + '-' + "%02d" % int(seq_number%20) + '-a' + str(i)
